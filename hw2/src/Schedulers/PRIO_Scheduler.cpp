...
bool PRIO_Scheduler::add_process(Process *p) {
    //if (dynamic prio < 0)
...
void PRIO_Scheduler::deactivate_process(Process *p) {
    activeQ->at(p->dynamic_priority).remove(p);
//    expiredQ->at(p->dynamic_priority).push_back(p);

}

void PRIO_Scheduler::showQ() {
    //{ [1][][0,2][]} : { [][][][]} :
    if(switched_queue){
...
Process *PRIO_Scheduler::get_from_runqueue(bool second_try) {
